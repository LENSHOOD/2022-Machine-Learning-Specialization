...
pkg_resources.require("gymnasium==1.2.3")

from gymnasium.envs.box2d.lunar_lander import (LunarLander, SCALE, FPS, VIEWPORT_W, VIEWPORT_H, LEG_DOWN, SIDE_ENGINE_AWAY, SIDE_ENGINE_HEIGHT, SIDE_ENGINE_POWER, MAIN_ENGINE_Y_LOCATION, MAIN_ENGINE_POWER)
import numpy as np
import math

class BidirectionalLunarLander(LunarLander):
    """
    LunarLander-v3 with bidirectional main engine thrust.

    Main engine action:
        action[0] ∈ [-1, +1]
        + → thrust along body +Y
        - → thrust along body -Y   (reverse thrust)

    Side engine unchanged.
    """
...
####################################################################################################################################
                ...
                ### main power changed to bidirectional
